...
it('handles a simple Observable', () => {
  expectObservable('RxSources.interval(1000)');
});

it('handles a higher order Observable', () => {
  expectObservable(
    'RxSources.interval(1000).pipe(RxOperators.groupBy(x => x % 2))'
  );
});

it('handles multiple statements', () => {
  expectObservable(
    `const first$ = RxSources.Observable.create(observer => {
      setTimeout(() => observer.next('1'), 1000);
      setTimeout(() => observer.complete(), 3000);
    });
    const second$ = RxSources.Observable.create(observer => {
